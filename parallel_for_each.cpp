...
int main() {
	...
	for (size_t i = 0; i < 20; i++) {
		...
		sc.start();
		for (size_t i = 0; i < toIterate.size(); i++) {
			if (out[i] != toIterate[i] * 2)
				std::cout << "[!] Error : " << out[i] << " is not "
						  << toIterate[i] << " * 2 " << std::endl;
		}
		sc.stop("checking intégrity     : ");
		out.clear();
		sc.start();
		std::for_each(std::execution::par, toIterate.begin(), toIterate.end(),
					  [&](auto i) { out.push_back(i * 2); });
